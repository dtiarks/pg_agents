...
class GaussPolicy(object):
    def __init__(self,sess,env,name,input_plh,action_plh,params,train=True,std_val=None):
        self.params=params
        self.sess=sess
        self.name=name
        self.env=env
        ...
        print("Initializing gaussian policy with action dim",self.params["actionsize"])
    ...
    def buildLowDimNet(self,input_layer):
        
        with tf.name_scope(self.name):
            with tf.name_scope('fc1'):
                self.W_fc1 = self._weight_variable([self.params["obssize"], 60],"W_fc1",vals=(-1./np.sqrt(self.params["obssize"]),1./np.sqrt(self.params["obssize"])))
            ...
            #     h_fc3 = tf.nn.tanh(tf.matmul(h_fc2, self.W_fc3) + self.b_fc3)
                
                
            with tf.name_scope('output'):
                self.W_fc4 = self._weight_variable([60, self.params["actionsize"]],"W_out",vals=(-0.01,np.sqrt(1/60.)))
